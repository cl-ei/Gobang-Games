...
def gameStart(screen,windows=(500,500)):
...
def gameOver(screen,windows=(500,500),score = 100):    
...
while True:
    ...
    if game_conflict == 1:
        new_head = [clsnake.position_obs[0],clsnake.position_obs[1]]
        clsnake.update_obs()
        clsnake.add_tlg()
    elif game_conflict == 2:
        clsnake.update_display(screen)
        gameOver(screen,(windows_x,windows_y),clsnake.length*100)
    else:
        pass
    #update display
    clsnake.update_display(screen)    
